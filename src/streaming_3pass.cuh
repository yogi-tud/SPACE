...
template <typename T> float launch_async_streaming_3pass(T* d_input, uint8_t* d_mask, T* d_output, uint64_t N, int stream_count)
{
    if (stream_count < 1) {
        stream_count = 1;
    }
    ...
    for (int i = 0; i < stream_count; i++) {
        ...
        // launch popc for i
        kernel_3pass_popc_none_monolithic<<<popc1_blockcount, popc1_threadcount, 0, streams[i]>>>(
            d_mask + bytes_per_stream * i, d_pss + chunks_per_stream * i, chunk_length32, chunks_per_stream);
        // launch pss for i
        ...
        kernel_3pass_popc_none_monolithic<<<popc2_blockcount, popc2_threadcount, 0, streams[i]>>>(
            d_mask + bytes_per_stream * i, d_popc + chunks1024_per_stream * i, 1024 / 32, chunks1024_per_stream);
        ...
        kernel_3pass_proc_true_striding<proc_threadcount, T, true><<<proc_blockcount, proc_threadcount, 0, streams[i]>>>(
            d_input + elems_per_stream * i, d_output, d_mask + bytes_per_stream * i, d_pss + chunks_per_stream * i,
            d_popc + chunks1024_per_stream * i, chunk_length, chunks_per_stream, chunk_count_p2, d_pss_totals + i);
    }
...
template <typename T> float launch_sync_streaming_3pass(T* d_input, uint8_t* d_mask, T* d_output, uint64_t N, int stream_count)
{
    if (stream_count < 1) {
        stream_count = 1;
    }
    ...
    for (int j = 0; j < stream_count + 2; j++) {
        ...
        if (i >= 0 && i < stream_count) {
            if (i > 0) {
                CUDA_TRY(cudaMemcpyAsync(
                    &h_pss_total, d_pss_total, sizeof(uint32_t), cudaMemcpyDeviceToHost, streams[i - 1])); // copy over old d_pss_total for writeout
        ...
        if (i >= 0) {
            ...
            kernel_3pass_popc_none_monolithic<<<popc2_blockcount, popc2_threadcount, 0, streams[i]>>>(
                d_mask + bytes_per_stream * i, d_popc + chunks1024_per_stream * i, 1024 / 32, chunks1024_per_stream);
            kernel_3pass_proc_true_striding<proc_threadcount, T, true><<<proc_blockcount, proc_threadcount, 0, streams[i]>>>(
                d_input + elems_per_stream * i, d_output + out_count, d_mask + bytes_per_stream * i, d_pss + chunks_per_stream * i,
                d_popc + chunks1024_per_stream * i, chunk_length, chunks_per_stream, chunk_count_p2, NULL);
        }
